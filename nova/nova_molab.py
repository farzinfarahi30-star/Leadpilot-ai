...
RAW = "https://raw.githubusercontent.com/farzinfarahi30-star/Leadpilot-ai/nova-gpu/nova/"
FILES = {
...
This notebook uses the existing Nova source, streams FineWeb, resumes checkpoints, then runs SFT, preference tuning, evaluation, and final promotion.
...
**Parameter target:** `{27_795_968:,}`")
    assert cuda and count >= 1, "Attach the NVIDIA GPU in molab before starting training."
...
def _(run, ROOT):
    ...
    ins = ROOT / "checkpoints/nova-instruct.pt"
    final = ROOT / "checkpoints/nova-final.pt"
    if not ck.exists() or ck.stat().st_size < 1_000_000:
        ...
        print("Pretraining checkpoint already exists; resuming only if incomplete.", flush=True)
        call([sys.executable, "train_hf_stream.py", "--dataset", "HuggingFaceFW/fineweb_100BT", "--steps", "1526", "--block-size", "1024", "--layers", "8", "--heads", "8", "--kv-heads", "4", "--embd", "512", "--vocab-size", "8192", "--batch-size", "2", "--grad-accum", "16", "--lr", "3e-4", "--checkpoint-every", "50", "--max-seconds", "39000", "--tokenizer-chars", "5000000", "--out", "checkpoints/nova-stream.pt", "--resume", "checkpoints/nova-stream.pt", "--grad-checkpoint"])
    ...
    model.eval()
    checks=[]
    for prompt in ["User: Explain what a variable is.\nAssistant:","User: Write a tiny Python loop.\nAssistant:","User: What is the purpose of testing?\nAssistant:"]:
        ids=torch.tensor([tok.encode(prompt)[-model.block_size:]],dtype=torch.long)
        out=model.generate(ids,48,temperature=.7,top_k=40)[0].tolist()
        text=tok.decode(out)
        checks.append(text[:180])
    safe_markers = ["bomb","explosive","malware","password cracking"]
    final_payload = {
