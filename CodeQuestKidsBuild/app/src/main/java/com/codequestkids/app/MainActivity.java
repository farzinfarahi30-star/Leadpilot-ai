...
public class MainActivity extends AppCompatActivity {
    ...
    private void showHome() {
        ...
        root.addView(text("Your progress", 20, true));
        root.addView(text(
        ...
        root.addView(text(
                "Offline features: age-track selection, missions, XP, progress, and saved coding projects. "
                        + "The official online academy is available when connected.",
                14,
                false
        ));
    }
    ...
    private void showMissions() {
        ...
        for (int i = 0; i < list.length; i++) {
            ...
            String label = (i < done ? "✓ " : "") + list[i] + "  +25 XP";
            root.addView(button(label, v -> {
    ...
    private void showCodingLab() {
        screen = 2;
        page("Coding Lab", "Write JavaScript and save your project locally.");

        codeEditor = new EditText(this);
        codeEditor.setText(prefs.getString(
                "code_" + track,
                "const player = { name: "Nova", score: 0 };\n"
                        + "player.score += 10;\n"
                        + "console.log(player.name, player.score);"
        ));
        codeEditor.setTextSize(15);
        codeEditor.setGravity(Gravity.TOP | Gravity.START);
        codeEditor.setTypeface(android.graphics.Typeface.MONOSPACE);
        codeEditor.setMinLines(12);
        codeEditor.setPadding(16, 16, 16, 16);

        root.addView(codeEditor, new LinearLayout.LayoutParams(-1, -2));

        root.addView(button("Save Project", v -> {
            prefs.edit().putString("code_" + track, codeEditor.getText().toString()).apply();
