...
async function runAgent(key,domain,capability,shared){
  ...
  try{
    ...
    }else if(key==='sales-expert' && process.env.NOVA_MAIL_TEST_TO){
      const mail=await sendEmail({
        provider:process.env.NOVA_MAIL_PROVIDER||'gmail',
        to:process.env.NOVA_MAIL_TEST_TO,
        from:process.env.NOVA_MAIL_TEST_FROM||process.env.GMAIL_FROM||process.env.GMAIL_ACCOUNT,
        ...
        approved:process.env.NOVA_MAIL_TEST_APPROVED==='true'
      });
      evidence.push({source:'Nova Mail',observation:mail.sent?'Email accepted by the configured provider':('Email not sent: '+(mail.reason||'provider failure')),trace:mail.messageId||mail.provider||'provider response'});
      return {...base,status:mail.sent?'completed':'failed',confidence:mail.sent?100:0,evidence,recommendation:mail.sent?'Return verified outbound-email evidence to Boss.':'Configure and explicitly approve the Nova Mail provider before claiming outbound email works.',finishedAt:new Date().toISOString(),email:mail};
